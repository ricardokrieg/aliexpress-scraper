...
class Scraper
  ...
  def self.scrape_products(category)
    ...
    puts "[#{category[:name]}] Scraping #{products.count.to_i} products".yellow

    products.each do |product|
      begin
        self.scrape_product(product, category)
  ...
  def self.scrape_product(product, category)
    ...
    puts "[#{category[:name]}] #{product[:aliexpress_id]}".green
  end
end